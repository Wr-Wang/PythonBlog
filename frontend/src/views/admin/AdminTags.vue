...
async function save() {
  ...
  try {
    if (editingId.value == null) {
      ...
      await updateTag(editingId.value, { name, slug });
    }
...
function askDel(r) {
  delRow.value = r;
  delMsg.value = `确定删除标签「${r.name}」（id=${r.id}）？`;
  delOpen.value = true;
}
...
async function confirmDel() {
  ...
  try {
    await deleteTag(delRow.value.id);
    delOpen.value = false;
    await load();
  } catch (e) {
...
<template>
  <div class="admin-page">
    <div class="admin-toolbar">
      <p class="admin-toolbar-desc">文章可挂多个标签；slug 建议简短且唯一，便于筛选与展示。</p>
    ...
    <div v-else class="admin-table-scroll">
      ...
      <p v-if="!rows.length" class="admin-table-empty">暂无标签。</p>
    </div>
    ...
    <ConfirmDialog :open="delOpen" title="删除标签" :message="delMsg" @close="delOpen = false" @confirm="confirmDel" />
  </div>
</template>
